...
{
    /// <summary>
    /// 可编辑图表数据，和行为树数据一致，这里只是方便编辑器中去操控行为树的数据。
    /// </summary>
    public class BTEditorGraph : ScriptableObject
    {
        ...
        {
            get { return m_isBehaviourTreeReadOnly || EditorApplication.isPlaying; }
        }

        public int Depth
        {
            get { return Mathf.Max(m_rootStack.Count - 1, 0); }
        }
