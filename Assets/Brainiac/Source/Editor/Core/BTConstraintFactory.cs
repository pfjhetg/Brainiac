...
namespace BrainiacEditor
{
    ...
    {
        private static List<Brainiac.Tuple<Type, string>> m_constraintMenuPaths;
        ...
        static BTConstraintFactory()
        {
            ...
            m_constraintMenuPaths = new List<Brainiac.Tuple<Type, string>>();
            foreach (Type type in assembly.GetTypes().Where(t => t.IsSubclassOf(constraintType)))
            {
                ...
                if (attributes.Length > 0)
                {
                    ...
                    m_constraintMenuPaths.Add(new Brainiac.Tuple<Type, string>(type, attribute.MenuPath));
                }
            }
        }
