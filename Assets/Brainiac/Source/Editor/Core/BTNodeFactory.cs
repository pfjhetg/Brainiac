...
namespace BrainiacEditor
{
    ...
    {
        private static List<Brainiac.Tuple<Type, string>> m_nodeMenuPaths;
        ...
        static BTNodeFactory()
        {
            ...
            m_nodeMenuPaths = new List<Brainiac.Tuple<Type, string>>();
            foreach (Type type in assembly.GetTypes().Where(t => t.IsSubclassOf(nodeType)))
            {
                ...
                if (attributes.Length > 0)
                {
                    ...
                    m_nodeMenuPaths.Add(new Brainiac.Tuple<Type, string>(type, attribute.MenuPath));
                }
            }
        }
