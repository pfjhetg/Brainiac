...
namespace BrainiacEditor
{
    ...
    {
        private static List<Brainiac.Tuple<Type, string>> m_serviceMenuPaths;
        ...
        static BTServiceFactory()
        {
            ...
            m_serviceMenuPaths = new List<Brainiac.Tuple<Type, string>>();
            foreach (Type type in assembly.GetTypes().Where(t => t.IsSubclassOf(serviceType)))
            {
                ...
                if (attributes.Length > 0)
                {
                    ...
                    m_serviceMenuPaths.Add(new Brainiac.Tuple<Type, string>(type, attribute.MenuPath));
                }
            }
        }
